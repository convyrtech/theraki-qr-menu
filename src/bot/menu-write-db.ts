...
export async function setHidden(entryId: number, hidden: boolean, actorId: number): Promise<void> {
  const sql = db();
  const rows = (await sql.query(`SELECT name, is_hidden FROM entries WHERE id=$1 AND NOT is_deleted`, [
    entryId,
  ])) as unknown as { name: string; is_hidden: boolean }[];
  if (!rows.length) throw new Error("Позиция не найдена.");
  await sql.query(`UPDATE entries SET is_hidden=$2, updated_at=now() WHERE id=$1`, [entryId, hidden]);
  await audit(sql, actorId, hidden ? "hide" : "unhide", entryId, {
    name: rows[0].name,
...
export async function setPrice(entryId: number, price: number, actorId: number): Promise<void> {
  ...
  const sql = db();
  const rows = (await sql.query(`SELECT name, price FROM entries WHERE id=$1 AND NOT is_deleted`, [
    entryId,
  ])) as unknown as { name: string; price: number }[];
  if (!rows.length) throw new Error("Позиция не найдена.");
  await sql.query(`UPDATE entries SET price=$2, updated_at=now() WHERE id=$1`, [entryId, price]);
  await audit(sql, actorId, "price", entryId, { name: rows[0].name, old: rows[0].price, new: price });
}
...
export async function setText(
  ...
  if (!rows.length) throw new Error("Позиция не найдена.");
  await sql.query(`UPDATE entries SET ${col}=$2, updated_at=now() WHERE id=$1`, [entryId, value]);
  await audit(sql, actorId, "text", entryId, { name: rows[0].name, field, old: rows[0].old, new: value });
}
...
export async function setFlag(
  ...
  if (!rows.length) throw new Error("Позиция не найдена.");
  await sql.query(`UPDATE entries SET ${col}=$2, updated_at=now() WHERE id=$1`, [entryId, value]);
  await audit(sql, actorId, "flag", entryId, { name: rows[0].name, field: flag, old: rows[0].old, new: value });
}

/** Мягкое удаление / восстановление. */
export async function softDelete(entryId: number, actorId: number): Promise<void> {
  const sql = db();
  const rows = (await sql.query(`SELECT name FROM entries WHERE id=$1 AND NOT is_deleted`, [
    entryId,
  ])) as unknown as { name: string }[];
  if (!rows.length) throw new Error("Позиция не найдена или уже удалена.");
  await sql.query(`UPDATE entries SET is_deleted=true, updated_at=now() WHERE id=$1`, [entryId]);
  await audit(sql, actorId, "delete", entryId, { name: rows[0].name });
}

export async function restoreEntry(entryId: number, actorId: number): Promise<void> {
  const sql = db();
  const rows = (await sql.query(`SELECT name FROM entries WHERE id=$1 AND is_deleted`, [
    entryId,
  ])) as unknown as { name: string }[];
  if (!rows.length) throw new Error("Удалённая позиция не найдена.");
  await sql.query(`UPDATE entries SET is_deleted=false, updated_at=now() WHERE id=$1`, [entryId]);
...
export async function addEntry(
  ...
  const id = rows[0].id;
  await audit(sql, actorId, "add", id, { name: data.name, chapterId, price: data.price });
  return id;
}
